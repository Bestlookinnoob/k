...
local AimlockMode = "RightClick"
local CamlockTarget = "Head"
local SpamMessage = "in jesus name i pray amen"
...
local NormalHH = 0
local SpamDelay = 1
local AimbotVelocity = 10
local NewPredictionVelocity = 66
local GravGunDistance = 10
local CrouchSpeed = 8
local FlySpeed = 8
local WalkSpeed = 16
local NormalWs = 16
...
local BulletColour = ColorSequence.new(Color3.fromRGB(248,248,248)) -- technically not an int but we'll go with it
...
local BlacklistTable = {
	[481150894] = true; -- Thinks hes better than glee
	[137076073] = true; -- retard I slapped its on YouTube btw xdxd
	[481265818] = true; -- said he would dox me but wouldn't
	[362925188] = true; --[[
		slapping this kid https://cdn.discordapp.com/attachments/763580177942446080/763814783341297714/RobloxScreenShot20201007_201503013.png
		sitting like a dog: https://cdn.discordapp.com/attachments/763580177942446080/763814785317601310/RobloxScreenShot20201007_194915586.png
		slaps like 5-1: https://cdn.discordapp.com/attachments/763580177942446080/763814790590103582/RobloxScreenShot20201007_215337634.png
	]]
	[74977147] = true; -- shes tip ridin
...
local SettingsTable = {
	...
	AimbotVelocity = 10;
	CmdBarImage = "http://www.roblox.com/asset/?id=5864514503";
	CmdBarKey = "Quote"
}
...
coroutine.resume(coroutine.create(function()
	...
	Players:Chat("i smell like poop") -- new admin
	Players:Chat("hi im an idiot") -- legacy admin
end))
...
getgenv().runHotkeys = function(ConfigToRun)
	...
	CmdBarImage = RunSettings.CmdBarImage or "http://www.roblox.com/asset/?id=5864514503"
...
local function IsAUser(Player,Chat)
	if Chat == "i am poop" or Chat == "i am usin editedRUST" then 
...
Players.PlayerAdded:Connect(function(Player)
	...
	Player.CharacterAdded:Connect(function(C)
		...
		if Head then 
			...
			if Blacklist or Player:IsInGroup(5152759) or string.find(Player.Name:lower(),"lynx","Smoked Fma","Edel") then
...
AddCommand(function(Arguments)
	Instance.new('Tool',LP.Backpack).Name = "Pretty Gay ;/"
...
AddCommand(function()
	wait(0.6)
	ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("no not made| Join the cord at gg.YOULOSE","All")
...
AddCommand(function(Arguments)
	...
	Tool.Parent = GetChar()
end,"pos",{"grip"},"Changes your tool .Grip","[6 Numbers (Optional)]")
...
end,"aa",{"antiaim"},"Breaks camlock to an extent","[Spin/No Args]")
...
end,"fling",{},"This is really stupid and buggy but funny when it works","[No Args]")
...
end,"game",{},"Stupid gun animations (gunanim [1/2/off])","[1/2/off]")
...
end,"camlock",{"lockcam","cl","c"},"Different type of aimbot (Uses camera instead of the remote)","[Player Name]")
...
end,"afk",{},"First you were a skid, Now you're annoying with a simple use of this command!","[Player]")
...
end,"triggerbot",{"slap"},"triggerbot goes brrrrrrrrrrrrrrrr","[Player]")
...
AddCommand(function(Arguments)
	if Arguments[1] and Arguments[1] == "arm" then
...
end,"antife",{},"Makes FE Loop not work (Legacy for removing right arm)","[Arm (Optional)]")
...
AddCommand(function(Arguments)
	if game.PlaceId ~= 4669040 then return notif("Wont work","Streets Only",5,nil) end
	if Arguments[1] and tonumber(Arguments[1]) then
...
end,"fly",{"fs"},"Allows you to fly [Up/Down Speed]","[Up/Down Speed]/Speed")
...
AddCommand(function()
	...
	GetChar():BreakJoints()
end,"tpbypass",{"bypass"},"Teleportation Bypass (Allows Infinite FlySpeed etc)","[No Args]")
...
end,"aimtarget”,{“at”},”Changes the aim target [Head/Torso/HumanoidRootPart/Prediction]","[Head/Torso/HumanoidRootPart/Pre]")
...
end,"aimlock",{"aim","al"},"Aimbot (Different method than camlock)","[Player]")
...
AddCommand(function(Arguments)
	if Arguments[1] then 
		...
		elseif Arguments[1] == "default" then
			CmdBarImageLabel.Image = "http://www.roblox.com/asset/?id=5864514503”
		end
		updateHotkeys(ConfigurationFile)
	end
...
coroutine.resume(coroutine.create(function()
	...
	CmdsFrame.BackgroundColor3 = Color3.fromRGB(13,105,172)
	CmdsFrame.BorderColor3 = Color3.fromRGB(13,105,172)
	...
	CmdsLabel.BackgroundColor3 = Color3.fromRGB(123,182,232)
	CmdsLabel.BackgroundTransparency = 0.2
	CmdsLabel.BorderColor3 = Color3.fromRGB(123,182,232)
	...
	CmdsLabel.TextColor3 = Color3.fromRGB(123,182,232)
	...
	CmdsScrolling.BorderColor3 = Color3.fromRGB(123,182,232)
	...
	CmdBarTextBox.BorderColor3 = Color3.fromRGB(17,17,17)
	...
	ValuesFrame.BackgroundColor3 = Color3.fromRGB(242,243,243)
	...
	ValuesTextLabel.BorderColor3 = Color3.fromRGB(17,17,17)
	...
	HotkeysFrame.BackgroundColor3 = Color3.fromRGB(242,243,243)
	...
	HotkeysTextLabel.BorderColor3 = Color3.fromRGB(242,243,243)
	...
	KeysFrame.BorderColor3 = Color3.fromRGB(248,248,248)
	...
	AnyCmdButton.BorderColor3 = Color3.fromRGB(248,248,248)
	...
	KeysLabel.BorderColor3 = Color3.fromRGB(248,248,248)
	...
	CmdBarKeyLabel.BorderColor3 = Color3.fromRGB(248,248,248)
	...
	CmdBarKeyButton.BorderColor3 = Color3.fromRGB(248,248,248)
	...
	AnyCmdTextBox.BorderColor3 = Color3.fromRGB(248,248,248)
...
coroutine.resume(coroutine.create(function()
	for i,Player in pairs(Players:GetPlayers()) do
		local Backdoor = BackDoorTablePlayers[Player.UserId]
		local Blacklist = BlacklistTable[Player.UserId]
		if (Blacklist or Player:IsInGroup(5152759,7290645,7889535) or string.find(Player.Name:lower(),"lynx","J̶u̶j̶u̶ ̶F̶a̶m̶","S̷I̷N̷I̷S̷T̷E̷R̷")) and Player.character and Player.Character:FindFirstChild'Head' then
			Esp(Player.Character.Head,Player.Name,Color3.fromRGB(102,51,0),true)
			Player.CharacterAdded:Connect(function(C)
				local Head = C:WaitForChild'Head'
				if Head then 
...
notif("Edited Cy V3!!","gang gang " .. string.format("%.6f",tick()-Tick) .. " seconds\n(Discord: gg.YOULOSE)",10,"rbxassetid://5864514503") -- string.format remains superior - Slays.
notif("That ass scrumptious v3","wowzers nigga",10,nil)
